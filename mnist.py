...
def load_data(one_hot=True, reshape=None, validation_size=10000):
    ...
    x_val = x_tr[-validation_size:]
    y_val = y_tr[-validation_size:]
    ...
    if one_hot:
        y_tr, y_te, y_val = [to_one_hot(y) for y in (y_tr, y_te, y_val)]

    if reshape:
        x_tr, x_te, x_val = [x.reshape(*reshape) for x in (x_tr, x_te, x_val)]
